...
#    cmp.percent = area of water / area of land + area of water
  ...
  def Aggregate5.Categorize(lsrsArray)
    ...
    for cmp in lsrsArray do
      if cmp.soil.SuitabilityClass == "NotRated" then # step 1.1
        # Populate water bodies category by adding to existing percent
        category.NotRated.push cmp
      elsif cmp.soil.DrainagePercentReduction > 60 then # step 1.2 - drainage component
    ...
    for cmp in domplusArray
      if cmp.soil.FinalSoilRating  >  domSoilRating - 15 and domLandscapeRating - 30  <  cmp.landscape.FinalRating and  cmp.landscape.FinalRating <  domLandscapeRating  + 30 then
  ...
  def Aggregate5.NotRated(notRatedArray)
  ...
  def Aggregate5.Soil(soilArray)
    ...
    if soilArray != []
      ...
      for cmp in soilArray
        ...
        soilRatingSum = soilRatingSum + cmp.soil.FinalSoilRating * cmp.percent
        landscapeRatingSum = landscapeRatingSum + cmp.landscape.FinalRating * cmp.percent
      end
  ...
  def Aggregate5.MostLimitingFactor(factorHash)
    # Step 4.1
  ...
  def Aggregate5.SummarizeSubfactors(category, sumPercent)
    if sumPercent > 0 then
      ...
      for cmp in category do
        if cmp.landscape.SlopeDeduction != nil then subfactorHash['landscapeT'] += cmp.landscape.SlopeDeduction * cmp.percent end
        if cmp.landscape.StoninessPercentReduction != nil then subfactorHash['landscapeP'] += cmp.landscape.StoninessPercentReduction * cmp.percent end
        if cmp.landscape.CoarseFragmentPercentReduction != nil then subfactorHash['landscapeP'] += cmp.landscape.CoarseFragmentPercentReduction * cmp.percent end # add Gravel to Stoniness to create P summary
        if cmp.landscape.WoodPercentReduction != nil then subfactorHash['landscapeJ'] += cmp.landscape.WoodPercentReduction * cmp.percent end
        if cmp.landscape.PatternPercentReduction != nil then subfactorHash['landscapeK'] += cmp.landscape.PatternPercentReduction * cmp.percent end
        if cmp.landscape.FloodingPercentReduction != nil then subfactorHash['landscapeI'] += cmp.landscape.FloodingPercentReduction * cmp.percent end
        if cmp.soil.MoistureDeficitDeduction != nil then subfactorHash['organicM'] += cmp.soil.MoistureDeficitDeduction * cmp.percent end
        if cmp.soil.TemperatureDeduction != nil then subfactorHash['organicZ'] += cmp.soil.TemperatureDeduction * cmp.percent end
        if cmp.soil.DrainagePercentReduction != nil then subfactorHash['organicW'] += cmp.soil.DrainagePercentReduction * cmp.percent end
        maxDeduction= [cmp.soil.SurfaceSalinityDeduction, cmp.soil.SubsurfaceSalinityDeduction].compact.max
        if maxDeduction != nil then subfactorHash['organicN'] += maxDeduction * cmp.percent end
        maxDeduction= [cmp.soil.SurfaceStructureDeduction,cmp.soil.SubsurfaceStructureDeduction].compact.max
        if maxDeduction != nil then subfactorHash['organicB'] += maxDeduction * cmp.percent end
        maxDeduction = [cmp.soil.SurfaceReactionDeduction,cmp.soil.SubsurfaceReactionDeduction].compact.max
        if maxDeduction != nil then subfactorHash['organicV'] += maxDeduction * cmp.percent end
        if cmp.soil.SubsurfaceSubstrateDeduction != nil then subfactorHash['organicG'] += cmp.soil.SubsurfaceSubstrateDeduction * cmp.percent end
        if cmp.soil.MoistureDeduction != nil then subfactorHash['mineralM'] += cmp.soil.MoistureDeduction * cmp.percent end
        if cmp.soil.DrainagePercentReduction != nil then subfactorHash['mineralW'] += cmp.soil.DrainagePercentReduction * cmp.percent end
        maxDeduction = [cmp.soil.SurfaceDeductionD,cmp.soil.SubsurfaceImpedenceDeduction].compact.max
        if maxDeduction != nil then subfactorHash['mineralD'] += maxDeduction * cmp.percent end
        if cmp.soil.SurfaceDeductionF != nil then subfactorHash['mineralF'] += cmp.soil.SurfaceDeductionF * cmp.percent end
        if cmp.soil.SurfaceDeductionE != nil then subfactorHash['mineralE'] += cmp.soil.SurfaceDeductionE * cmp.percent end
        maxDeduction = [cmp.soil.SurfaceSalinityDeduction,cmp.soil.SubsurfaceSalinityDeductionInterim].compact.max
        ...
        if cmp.soil.OrganicDeductionO != nil then subfactorHash['mineralO'] += cmp.soil.OrganicDeductionO * cmp.percent end
        maxDeduction = [cmp.soil.SurfaceSodicityDeduction,cmp.soil.SubsurfaceSodicityDeduction].compact.max
  ...
  def Aggregate5.DropBelow20(subfactorHash)
  ...
  def Aggregate5.DropAH(subfactorHash, rating)
  ...
  def Aggregate5.DropAM(subfactorHash)
  ...
  def Aggregate5.DeleteSubfactors(subfactorHash, allSubfactorKeys)
    # part of Step 5.4.1
  ...
  def Aggregate5.DropWithinFactor(subfactorHash)
    # Step 5.4.1  within factor comparison
    if subfactorHash != {} then
      # Climate Factor
      subfactorHash = Aggregate5.DeleteSubfactors(subfactorHash, ['A','H'])
      # Landscape Factor
      subfactorHash = Aggregate5.DeleteSubfactors(subfactorHash, ['T','P','J','K','I'])
      # Soil Factor
      subfactorHash = Aggregate5.DeleteSubfactors(subfactorHash, ['M','Z','W','N','B','V','G','D','F','E','O','Y'])
    end
    return subfactorHash
  end
  
  def Aggregate5.DropWithinFactorOLD(subfactorHash)
    # Step 5.4.1  within factor comparison
    if subfactorHash != {} then
      # Climate Factor
  ...
  def Aggregate5.DropOtherFactors(subfactorHash, category)
...
#    threshold = 100 - (100 - category[category['MostLimitingFactor'] + "Rating"]) * 0.33
  ...
  def Aggregate5.PrimarySubclass(subfactorHash, category)
  ...
  def Aggregate5.AdditionalSubclasses(subfactorHash, factorHash)
  ...
  def Aggregate5.FinalClass(subfactorHash, factorHash)
  ...
  def Aggregate5.rating(drainageFactorHash,dominantFactorHash,dissimilarFactorHash,perdecimNotRated)
