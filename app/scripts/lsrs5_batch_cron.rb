...
log = File.new(@batchDir + "lsrsbatchlog", "a")
Dir.chdir(@batchDir + "pending/")
pendingJobs = Dir.glob("*.yml").sort
if pendingJobs.size > 0
	puts "Doing it"       ############## REMOVE IN PRODUCTION
  # at least one job is pending
  if Dir.glob(@batchDir + "processing/*.yml") == []
    # and no jobs are processing, so run the next job
		puts "running a job"       ############## REMOVE IN PRODUCTION
    ...
    @controlFilename = @batchDir + "processing/" + nextJob
		FileUtils.cp(@batchDir + "pending/" + nextJob, @controlFilename) ############# CHANGE cp BACK TO mv IN PRODUCTION !!!
    ...
    @climateTableName = @control['ClimateTable']  # TODO: change from MySQL table row to Redis key name
		FileUtils.cp(@batchDir + "pending/" + nextJob, @controlFilename)  ################# REMOVE IN PRODUCTION
...
# check to see if climate data is in Redis


puts "Framework = " + @frameworkName  ################# REMOVE IN PRODUCTION
puts "@climateTableName = " + @climateTableName  ################# REMOVE IN PRODUCTION
puts File.exist? 
puts
stop  

		# load climate data into Redis
		
#    - load the climate indices file into redis if necessary, or update the expiration time
    redis.expire(key, 10000)
#    - generate the climate indices file if necessary
#    - regenerate the climate indices file if source file is newer
...
#      outputXmlFile << open(lsrsURL).read()
    ...
    # delete control file
    FileUtils.mv(@controlFilename, @batchDir + "results/" + nextJob[0..19] + "/control.yml")
