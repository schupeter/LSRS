...
xml.LSRS do
  xml.Request do
    xml.FrameworkName(@rating.polygon.frameworkName)
    xml.Database(@rating.polygon.databaseTitle)
    xml.Polygon(@rating.polygon.polyId)
    xml.Crop(@rating.crop)
  ...
  xml.Climate do
    ...
    xml.Moisture do
      ...
      xml.DeductionUsed(@climate.A_deduct)
      xml.Value(@climate.A)
    end
    xml.Temperature do
      xml.HF do
