# RUN USING:   script/console < app/controllers/lsrs_test_aggregate.rb
...
@cmpTableName = "bc_lowerfraser_soil_v2x0_cmp".capitalize
@polyId = "BC5004581"
@crop = "corn"
@cropHash.store("CROP", "corn")
@climateTableName = "Bc_lowerfraser_soil_v2x0_climate1961x90_nlwis_slcv3x0".capitalize
@response = "Details"

# validate request parameters are present and valid
if !(defined? @cmpTableName) then @exceptionCode = "MissingParameterValue"; @exceptionParameter = "CmpTable"; render :action => 'Error_response', :layout => false and return and exit 1 end
...
# crop record
if (Lsrs_climateparam.find(:all, :conditions => {:crop => @crop}).size !=  1 ) then @exceptionCode = "InvalidParameterValue"; @exceptionParameter = "CROP"; @exceptionParameterValue = @crop; render :action => 'Error_response', :layout => false and return and exit 1 end
# component table
@cmpTableMetadata = LsrsCmp.find(:all, :conditions=>{"WarehouseName" => @cmpTableName})[0]
if @cmpTableMetadata ==  nil then @exceptionCode = "InvalidParameterValue"; @exceptionParameter = "CMPTABLE"; @exceptionParameterValue = @cmpTableName; render :action => 'Error_response', :layout => false and return and exit 1 end
...
#    if @slcCmp == true then
lsrsInputURL = "http://lsrs.gis.agr.gc.ca/lsrsinput/service?POLY_ID=" + @landscapeId +"&DATA=landscape"
lsrsInput = open(lsrsInputURL).read().to_libxml_doc.root
