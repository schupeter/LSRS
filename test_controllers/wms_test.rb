...
class Directory
attr_accessor  :tmpDirName, :tmpMapDir, :tmpCgiDir, :tmpHtmDir
end
directory = Directory.new
# create directories
directory.tmpDirName = tmpDirName
# MapDir and HtmDir are the same, but 
directory.tmpMapDir = "/usr/local/httpd/lsrs/public/batch/results/" + tmpDirName
directory.tmpHtmDir = "/usr/local/httpd/lsrs/public/batch/results/" + tmpDirName
directory.tmpCgiDir = "/usr/local/httpd/lsrs/cgi-bin/" + tmpDirName
# create CGI-BIN directory because it won't yet exist
Dir.mkdir(directory.tmpCgiDir)


# CREATE SHAPEFILE  Wms_create.Shapefile(attributeHash, rowsXmlArray, xwarehouseName, xoriginalPAT, xall_frameworks, tmpMapDir, classificationArray, exceptionsArray)
require 'dbf'
require "#{Rails.root.to_s}/app/helpers/dbf-helper"

# load the original PAT file
#table = DBF::Table.new("/usr/local/httpd/mapdata/bc/okanagan/soil/v2x0/bc_okanagan_soil_v2x0.dbf")
